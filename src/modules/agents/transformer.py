...
class TransformerAggregationAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.q_trans = nn.Linear(args.emb*(1+args.enemy_num+args.ally_num),args.emb)
        self.q_linear = nn.Linear(args.emb, args.n_actions)

    def init_hidden(self):
        # make hidden states on same device as model
        return torch.zeros(1, self.args.emb).cuda() if self.args.use_cuda else torch.zeros(1, self.args.emb)
    ...
    def  forward(self, inputs, hidden_state, task_enemy_num, task_ally_num):
        ...
        h = outputs[:,-1:,:]
        outputs = outputs.reshape(-1,(1+task_enemy_num+task_ally_num)*self.args.emb)
