...
from modules.intrinsic.cds_network import Predict_Network1, Predict_Network1_combine
...
class CDS_Learner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if self.args.use_cuda:
            self.eval_predict_withid.cuda()
            self.target_predict_withid.cuda()

            self.eval_predict_withoutid.cuda()
            self.target_predict_withoutid.cuda()
